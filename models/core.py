...
# In coltran/models/core.py or layers.py

class CrossAttentionBlock(layers.Layer):
    """A Transformer block that handles different feature dims for cross-attention."""

    def __init__(self, hidden_size, num_heads, text_feature_dim, dropout_rate=0.1, **kwargs):
        ...
        # Self-Attention for the image features (no change here)
        self.self_attention = layers.MultiHeadAttention(
            num_heads=num_heads, key_dim=hidden_size // num_heads, output_shape=hidden_size)
        self.self_attention_norm = layers.LayerNormalization()
        self.self_attention_dropout = layers.Dropout(dropout_rate)

        # --- FIX ---
        # Add Dense layers to project Key and Value from text dimension to image dimension
        self.key_proj = layers.Dense(hidden_size, name="cross_attn_key_proj")
        self.value_proj = layers.Dense(hidden_size, name="cross_attn_value_proj")
        # The query will be projected internally by the MHA layer, so we don't need a separate layer.

        # Cross-Attention layer. Note the output_shape to ensure it returns the correct dimension.
        self.cross_attention = layers.MultiHeadAttention(
            num_heads=num_heads, key_dim=hidden_size // num_heads, output_shape=hidden_size)
        # --- END FIX ---

        self.cross_attention_norm = layers.LayerNormalization()
        self.cross_attention_dropout = layers.Dropout(dropout_rate)

        # Feed-Forward Network (MLP) - no change here
    ...
    def call(self, inputs, training=False):
        ...
        # 1. Self-Attention Block (no change)
        self_attn_output = self.self_attention(query=image_features, value=image_features, key=image_features)
        self_attn_output = self.self_attention_dropout(self_attn_output, training=training)
        image_features = self.self_attention_norm(image_features + self_attn_output)

        # 2. Cross-Attention Block
        # --- FIX ---
        # Project the text features BEFORE passing them to the attention layer
        key = self.key_proj(text_features)  # Shape: (B, TxtSeqLen, 512)
        value = self.value_proj(text_features)  # Shape: (B, TxtSeqLen, 512)

        # Now all inputs to cross_attention have the same feature dimension (512)
        cross_attn_output = self.cross_attention(
            query=image_features,  # Shape: (B, ImgSeqLen, 512)
            value=value,  # Shape: (B, TxtSeqLen, 512)
            key=key  # Shape: (B, TxtSeqLen, 512)
        )
        # --- END FIX ---

        cross_attn_output = self.cross_attention_dropout(cross_attn_output, training=training)
        image_features = self.cross_attention_norm(image_features + cross_attn_output)

        # 3. Feed-Forward Block (no change)
        ffn_output = self.ffn(image_features)
        ffn_output = self.ffn_dropout(ffn_output, training=training)
        image_features = self.ffn_norm(image_features + ffn_output)
