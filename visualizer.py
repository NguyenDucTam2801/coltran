...
def export_logits_channels(logits_tensor, output_dir, prefix="logit_channel", use_matplotlib=True,cmap='viridis'):
    """
    ...
    for i in tqdm(range(num_channels)):  # Use tqdm for progress bar
        ...
        image_data_uint8 = tf.cast(normalized_data * 255.0, tf.uint8)
        ...
        try:
            if use_matplotlib:
                plt.imsave(filename, image_data_uint8.numpy(), cmap=cmap, vmin=0, vmax=255)
            else:
                # Use Pillow
...
def main(_):
    ...
    example_spatial_upsampled = tf.constant(np.random.randint(0, 256, size=(1, 256, 256, 3)),
                                            dtype=tf.int32)  # Or uint8
